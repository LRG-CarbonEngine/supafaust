...
#include "CarbonSystem.h"

#define FASTMODE_WS 3

#if defined (PLATFORM_NX)
extern System* g_System;
#else
extern CarbonSystem* g_System;
#endif

#if defined(HAS_SNES_BOOST) && defined (HAS_BOSSRUSH)
#define boostmodecheck if (g_System->GetCurrentBoostMode()|| g_System->m_BossRush->GetActive()) \
CPUM.timestamp += FASTMODE_WS; \
#elif defined(HAS_SNES_BOOST)
#define boostmodecheck if (g_System->GetCurrentBoostMode()) \
CPUM.timestamp += FASTMODE_WS; \
else 
#else
#define boostmodecheck
#endif 
...
bool CART_Init(Mednafen::Stream* fp, uint8 id[16], const int32 cx4_ocmultiplier, const int32 superfx_ocmultiplier, const bool superfx_enable_icache)
{
 bool IsPAL = false;
